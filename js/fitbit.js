    ...
    activityType_Array.forEach(function(value){
        ...
        if(!localstorageData){
            ...
            if ((newDate-oldDate) > sessionExpireTime) {
                ...
                popuplateFitbitData(value,temp.value);
            }
        }
    })
}
	...
	$.ajax({
		...
		success: function(response){
			...
			if(jsonResponse){
				...
				popuplateFitbitData(activityType,fitbitDataObject.value);
			}else{
...
function popuplateFitbitDataFromLocalStorage(activityType){
	...
	if(temp){
		popuplateFitbitData(activityType,temp.value);
	}else{
...
function popuplateFitbitData(activityType, data){
	// Add fitbit data to stats
	jQuery("#fitbitData").append('<div class="col-6"><h5>Average '+activityType+' Per Day:</h5><span id="'+activityType+'">'+data+' '+unitType_kvp[activityType]+'</span></div>');
}
